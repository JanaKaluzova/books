...
export const ConfirmDialog: FC<ConfirmDialogProps> = ({
  ...
  return (
    <Root>
      ...
      <Portal>
        <Overlay
          className={`${MODAL_BACKDROP} fixed inset-0 z-[110] animate-[fadeIn_150ms_ease]`}
        />
        <Content className="fixed left-1/2 top-1/2 z-[120] w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-3xl border border-surface-200 bg-white p-6 shadow-2xl animate-[scaleIn_150ms_ease]">
          ...
          <div className="mt-6 flex justify-end gap-3">
            <Cancel asChild>
              <button
                type="button"
                className="rounded-xl px-5 py-2.5 text-sm font-medium text-text-secondary transition-colors hover:bg-surface-100 hover:text-text-primary"
              >
                Cancel
              </button>
            </Cancel>
